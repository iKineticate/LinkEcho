...
#[allow(unused)]
pub fn change_all_links_icons(link_vec: &mut Vec<LinkProp>) -> Result<(), String> {
    // 存储 PowerShell 命令
    ...
    for path_buf in glob(&select_icons_folder_path).unwrap().filter_map(Result::ok) {
        ...
        for link_prop in link_vec.iter_mut() {
            ...
            // 比较图标名称与LNK名称的包含关系
            let link_name_lowercase = link_prop.name.clone().trim().to_lowercase();
            ...
            // 跳过相等关系且已使用该图标情况
            if let Some(path) = &link_prop.icon_location {
                if path == &icon_path {
                    continue;
                } else {    // 更新LinkProp结构体的图标路径
                    link_prop.icon_location = Some(String::from(icon_path.clone()));
...
pub fn restore_all_links_icons(link_vec: &mut Vec<LinkProp>) -> Result<&'static str, String> {
    ...
    // 遍历link_vec
    for link_prop in link_vec.iter_mut() {
        // 跳过未更换或扩展为uwp|app的快捷方式
        if link_prop.icon_status.is_none() || link_prop.target_ext == String::from("uwp|app") {
            continue;
        }
        let icon_path = link_prop.target_path.clone().unwrap_or(String::new());
        // 追加命令
        command.push_str(&format!(
            r#"
            $shortcut = $shell.CreateShortcut("{link_path}")
        ...
        link_prop.icon_location = Some(icon_path);
